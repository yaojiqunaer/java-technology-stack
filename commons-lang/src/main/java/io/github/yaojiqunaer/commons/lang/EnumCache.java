...
/**
 * 枚举缓存
 * <a href= "https://mp.weixin.qq.com/s/CLr5bcxsG7C8v6qSsagEbw"></a>
 */
public class EnumCache {

    private static final String ENUM_STATIC_METHOD = "values";
     ...
     * deprecated -> {@link EnumCache#registerByName(Class)}
     *
     * @param clazz
     * @param es
     * @param <E>
    ...
    public static <E extends Enum> void registerByName(Class<E> clazz) {
        try {
            Method values = clazz.getMethod(ENUM_STATIC_METHOD);
            Object invoke = values.invoke(null);
            if (invoke instanceof Object[] array) {
                Map<Object, Enum> map = new ConcurrentHashMap<>();
            ...
            throw new RuntimeException("reflect exception");
        }
    }
    ...
    public static <E extends Enum> void registerByValue(Class<E> clazz, EnumMapping<E> enumMapping) {
        ...
        try {
            ...
            if (invoke instanceof Object[] array) {
                ...
                for (Object item : array) {
                    if (item instanceof Enum e) {
                        Object value = enumMapping.value((E) e);
    ...
    /**
     * 以枚举转换出的任意值构建缓存，在枚举的静态块里面调用
     * deprecated -> {@link EnumCache#registerByValue(Class, EnumMapping)}
     *
     * @param clazz
     * @param es
     * @param enumMapping
    ...
    private static <E extends Enum> E find(Class<E> clazz, Object obj,
        ...
        if (map == null) {
            ...
            if (CACHE_BY_VALUE.equals(cache)) {
                msg = String.format(
                        "枚举%s还没有注册到枚举缓存中，请在%s.static代码块中加入如下代码 : EnumCache.registerByValue(%s.class, %s.values(), " +
                                "%s::getXxx);",
                        clazz.getSimpleName(),
                        clazz.getSimpleName(),
                        clazz.getSimpleName(),
        ...
        if (obj == null) {
            if (defaultEnum == null) {
                throw new IllegalArgumentException("cannot find any enum instance by " + obj);
            }
            return defaultEnum;
        }
